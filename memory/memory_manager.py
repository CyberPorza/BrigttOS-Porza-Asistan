...
# CyberPorza Long-Term Memory — Remembers user identity, preferences, and notes
...
def load_memory() -> dict:
    ...
    with _lock:
        try:
            ...
            print(f"[PORZA-MEM] ⚠️ Yükleme hatası: {e}")
            return _empty_memory()
...
def update_memory(memory_update: dict) -> dict:
    """Updates and saves changes to long-term memory."""
    ...
    if _recursive_update(memory, memory_update):
        save_memory(memory)
        print(f"[PORZA-MEM] 💾 Hafıza güncellendi: {list(memory_update.keys())}")
...
def format_memory_for_prompt(memory: dict | None) -> str:
    """Formats memory into a concise string for the AI prompt."""
    if not memory:
        return ""
