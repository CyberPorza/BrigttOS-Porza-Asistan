...
SYSTEM_NAME = "CYBERPORZA"
MODEL_BADGE = "CyberPorza Gelişmiş Siber Asistan"
...
class BrightosUI:
    def __init__(self, face_path, size=None):
        self.root = tk.Tk()
        self.root.title("CyberPorza — Gelişmiş Siber Asistan V1.0")
    ...
    def _draw(self):
        ...
        c.create_text(W // 2, 22, text="CYBERPORZA",
                      fill=C_PRI, font=("Courier", 18, "bold"))
        c.create_text(W // 2, 44, text="CyberPorza — Yerel Siber Bağlantı",
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="BURAK AI  ·  GİZLİ  ·  GÜVENLİ ERİŞİM")

    def write_log(self, text: str):
        self.typing_queue.append(text)
        tl = text.lower()
        self.status_text = ("İŞLENİYOR" if tl.startswith("siz:") or tl.startswith("you:")
                            else "YANITLIYOR" if tl.startswith("porza:") or tl.startswith("ai:")
    ...
    def _start_typing(self):
        if not self.typing_queue:
            ...
            if not self.speaking:
                self.status_text = "ÇEVRİMİÇİ"
            return
        self.is_typing = True
        text = self.typing_queue.popleft()
        tl   = text.lower()
        tag  = "you" if tl.startswith("siz:") or tl.startswith("you:") else "ai" if tl.startswith("porza:") or tl.startswith("ai:") else "sys"
    ...
    def wait_for_api_key(self):
        """Block until API key is saved (called from main thread before starting)."""
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(self.setup_frame, text="◈  SİSTEM BAŞLATMA GEREKLİ",
                 fg=C_PRI, bg="#00080d", font=("Courier", 13, "bold")).pack(pady=(18, 4))
        tk.Label(self.setup_frame,
                 text="CyberPorza'yı başlatmak için Gemini API anahtarını gir kankam.",
        ...
        tk.Button(
            self.setup_frame, text="▸  SİSTEMİ BAŞLAT",
    ...
    def _save_api_keys(self):
        ...
        self.status_text = "ÇEVRİMİÇİ"
        self.write_log("SYS: Sistemler başlatıldı. CyberPorza çevrimiçi kankam.")
...
class Brightos3DUI:
    ...
    def write_log(self, text: str):
        ...
        if self.window:
            try:
                ...
                tl = text.lower()
                if "porza:" in tl or "ai:" in tl:
                    clean_text = text.split(":", 1)[1].strip() if ":" in text else text
                    self.window.evaluate_js(f"if(window.py_speak) window.py_speak({json.dumps(clean_text)})")
                
                # Update status based on message type
                status = "İŞLENİYOR" if "siz:" in tl or "you:" in tl else "YANITLIYOR" if "ai:" in tl or "porza:" in tl else "ÇEVRİMİÇİ"
    ...
    def start_speaking(self):
        ...
        if self.window:
            try:
                self.window.evaluate_js("if(window.py_setStatus) window.py_setStatus('KONUŞUYOR')")
            except:
                pass
    ...
    def stop_speaking(self):
        ...
        if self.window:
            try:
                self.window.evaluate_js("if(window.py_setStatus) window.py_setStatus('ÇEVRİMİÇİ')")
            except:
                pass
    ...
    def run(self):
        ...
        self.window = webview.create_window(
            "CYBERPORZA - Gelişmiş Siber Arayüz",
