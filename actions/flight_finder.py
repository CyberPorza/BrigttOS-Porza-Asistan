...
# CyberPorza — Flight Finder
#
# Searches for flights using Google Flights via browser_control.
# Results are spoken by CyberPorza. Optionally saved to Notepad or opened in browser.
#
# Flow:
...
def _search_flights_browser(
    ...
    print(f"[PORZA-FLIGHT] 🌐 Opening: {url}")
...
def _parse_flights_with_gemini(
    ...
    try:
        ...
        # Markdown hatasından kaçınmak için stringi böldük
        text     = re.sub(r"``" + r"`(?:json)?", "", text).strip().rstrip("`").strip()
        flights  = json.loads(text)
        return flights if isinstance(flights, list) else []
    except Exception as e:
        print(f"[PORZA-FLIGHT] ⚠️ Parse failed: {e}")
        return []
...
def _format_spoken(
    ...
    if not flights:
        return (
            f"Kankam, {date} tarihinde {origin} konumundan {destination} konumuna "
            f"hiç uçuş bulamadım. Sayfa tam yüklenmemiş olabilir."
        )

    lines = [f"{date} tarihinde {origin} - {destination} arası bulduğum uçuşlar şöyle kankam."]

    for i, f in enumerate(flights[:5], 1):
        airline   = f.get("airline",   "Bilinmeyen havayolu")
        ...
        stop_str  = "direkt uçuş" if stops == 0 else f"{stops} aktarmalı"
        price_str = f"{price} {currency}".strip() if price else "fiyat bilgisi yok"
        dur_str   = f", yolculuk süresi {duration}" if duration else ""

        lines.append(
            f"Seçenek {i}: {airline}, kalkış {departure}, varış {arrival}"
    ...
    if cheapest:
        lines.append(
            f"En ucuz seçenek {cheapest.get('price')} {cheapest.get('currency', '')} "
            f"fiyatıyla {cheapest.get('airline')} firmasına ait."
        )
...
def _format_notepad(
    ...
    lines = [
        "CYBERPORZA — Flight Search Results",
        "─" * 50,
...
def _save_to_notepad(content: str, origin: str, destination: str) -> str:
    ...
    print(f"[PORZA-FLIGHT] 💾 Saved: {filepath}")
...
def flight_finder(
    ...
    if not origin or not destination:
        return "Lütfen hem kalkış hem de varış noktasını belirt kankam."
    if not date_raw:
        return "Lütfen gidiş tarihini söyle kankam."

    date        = _parse_date(date_raw)
    return_date = _parse_date(return_raw) if return_raw else None

    if player:
        player.write_log(f"CYBER-FLIGHT: {origin} → {destination} on {date}")

    if speak:
        speak(f"{date} tarihinde {origin} konumundan {destination} konumuna uçuşları arıyorum kankam.")

    print(f"[PORZA-FLIGHT] ▶️ {origin} → {destination} | {date} | {cabin} | {passengers} pax")
    ...
    try:
        ...
        if not raw_text:
            return "Uçuş verilerini çekemedim kankam, sayfa tam yüklenmemiş olabilir."

        if speak:
            speak("Sonuçları analiz ediyorum.")
        ...
        if save and flights:
            ...
            result += f" Sonuçları masaüstüne kaydettim kankam: {saved_path}"

        return result

    except Exception as e:
        print(f"[PORZA-FLIGHT] ❌ Error: {e}")
        return f"Uçuş araması başarısız oldu kankam: {e}"