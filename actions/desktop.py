...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            print(f"[PORZA-DESK] 🤖 Asking Gemini: {actual_task}")
            if player:
                player.write_log("[CYBER-OPS] Generating action...")
            ...
            if code == "UNSAFE":
                ...
                print(f"[PORZA-DESK] ✅ Generated code:\n{code[:200]}")
    ...
    print(f"[PORZA-DESK] {result[:100]}")
    if player:
        player.write_log(f"[CYBER-OPS] {result[:60]}")
