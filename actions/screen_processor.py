...
# CyberPorza'nın Karakteri (Sistem Promptu)
SYSTEM_PROMPT = (
    "You are CyberPorza, an advanced elite AI assistant created by Burak. "
    ...
    "Be concise, smart, and helpful like a top-tier hacker's sidekick. "
    "Respond in maximum 2 short sentences. Speed is priority. "
    "Address the user as 'kankam' for a friendly and highly informal tone. "
...
def _get_camera_index() -> int:
    ...
    print("[PORZA-CAM] 🔍 No camera index in config. Auto-detecting...")
    best_index = 0
    ...
    for idx in range(6):
        ...
        if ret and frame is not None and frame.mean() > 5:
            best_index = idx
            print(f"[PORZA-CAM] ✅ Camera found at index {idx} — saving to config.")
            break
        else:
            print(f"[PORZA-CAM] ⚠️  Index {idx}: no valid frame (black or empty).")
    ...
    try:
        ...
        print(f"[PORZA-CAM] 💾 Camera index {best_index} saved to config.")
    except Exception as e:
        print(f"[PORZA-CAM] ⚠️  Could not save camera index: {e}")
...
class _LiveSession:
    ...
    def start(self, player=None):
        ...
        print("[PORZA-VISION] ✅ Vision session ready (no mic)")
    ...
    async def _main(self):
        ...
        while True:
            try:
                print("[PORZA-VISION] 🔌 Vision session connecting...")
                async with client.aio.live.connect(model=LIVE_MODEL, config=config) as session:
                    self._session = session
                    self._ready.set()
                    print("[PORZA-VISION] ✅ Vision session connected")
                ...
                print(f"[PORZA-VISION] ⚠️ Disconnected: {e} — reconnecting...")
                self._session = None
                self._ready.clear()
        ...
        while True:
            ...
            if self._session:
                ...
                try:
                    ...
                    print("[PORZA-VISION] ✅ Image sent")
                except Exception as e:
                    print(f"[PORZA-VISION] ⚠️ Send error: {e}")
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript_buf and self._player:
                        ...
                        if full:
                            self._player.write_log(f"CyberPorza: {full}")
                            print(f"[PORZA-VISION] 💬 {full}")
                    transcript_buf = []

        except Exception as e:
            print(f"[PORZA-VISION] ⚠️ Recv error: {e}")
            transcript_buf = []
...
def screen_process(
    ...
    if not user_text:
        print("[PORZA-VISION] ⚠️ No user_text provided.")
        return False

    angle = (parameters or {}).get("angle", "screen").lower().strip()
    print(f"[PORZA-VISION] angle={angle!r}  text={user_text!r}")
    ...
    try:
        if angle == "camera":
            ...
            print("[PORZA-VISION] 📷 Camera captured")
        else:
            image_bytes = _capture_screenshot()
            mime_type   = "image/jpeg" if _PIL_OK else "image/png"
            print("[PORZA-VISION] 🖥️ Screen captured")
    except Exception as e:
        import traceback; traceback.print_exc()
        print(f"[PORZA-VISION] ❌ Capture error: {e}")
        return False

    print(f"[PORZA-VISION] 📦 {len(image_bytes)} bytes → sending")
...
def warmup_session(player=None):
    ...
    try:
        ...
        print(f"[PORZA-VISION] ⚠️ Warmup error: {e}")


if __name__ == "__main__":
    print("[TEST] CyberPorza screen_processor.py v8 — image-only session")
    print("=" * 50)
