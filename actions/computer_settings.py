...
# CyberPorza System Control Module
#
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            print(f"[PORZA-SYS] 🔊 Volume → {value}%")
            return
        except Exception as e:
            print(f"[PORZA-SYS] ⚠️ pycaw failed: {e}")
    elif _OS == "Darwin":
...
- Return ONLY the JSON object, no explanation, no markdown."""
    ...
    try:
        ...
        # SORUN ÇIKARAN SATIR DÜZELTİLDİ (Markdown hatasını atlatmak için string bölündü):
        text = __import__("re").sub(r"``" + r"`(?:json)?", "", text).strip().rstrip("`").strip()
        return json.loads(text)
    except Exception as e:
        print(f"[PORZA-SYS] ⚠️ Intent detection failed: {e}")
...
def computer_settings(
    ...
    print(f"[PORZA-SYS] ⚙️ Action: {action}  Value: {value}")
    if player:
        player.write_log(f"CYBER-SYS: Executing {action}...")
