...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"porza_script{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"porza_debug_{int(time.time())}.png"
        screenshot = pyautogui.screenshot()
        screenshot.save(str(screenshot_path))
        print(f"[PORZA-DEV] 📸 Screenshot: {screenshot_path}")
        return screenshot_path
    except Exception as e:
        print(f"[PORZA-DEV] ⚠️ Screenshot failed: {e}")
        return None
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    if player:
        player.write_log("CYBER-CODE: Build started...")

    lang = language or "python"

    try:
        code, path = _write(description, lang, output_path, player)
        print(f"[PORZA-DEV] ✅ Written: {path}")
    except Exception as e:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        print(f"[PORZA-DEV] 🔄 Attempt {attempt}/{MAX_BUILD_ATTEMPTS}")
        if player:
            player.write_log(f"CYBER-CODE: Attempt {attempt}...")
        ...
        print(f"[PORZA-DEV] ⚠️ Error on attempt {attempt}, fixing...")
        if player:
            player.write_log(f"CYBER-CODE: Fixing (attempt {attempt})...")
...
def _write_action(description, language, output_path, player) -> str:
    ...
    if player:
        player.write_log("CYBER-CODE: Writing code...")
    try:
...
def _edit_action(file_path, instruction, player) -> str:
    ...
    if player:
        player.write_log("CYBER-CODE: Editing file...")
...
Updated code:"""
    ...
    print(f"[PORZA-DEV] ✅ Edited: {file_path}")
...
def _explain_action(file_path, code, player) -> str:
    ...
    if player:
        player.write_log("CYBER-CODE: Analyzing code...")
...
def _run_action(file_path, args, timeout, player) -> str:
    ...
    if not p.exists():
        return f"File not found: {file_path}"
    if player:
        player.write_log(f"CYBER-CODE: Running {p.name}...")
...
def _optimize_action(file_path, code, language, output_path, player) -> str:
    ...
    if player:
        player.write_log("CYBER-CODE: Optimizing code...")
...
Optimized code:"""
    ...
    print(f"[PORZA-DEV] ✅ Optimized: {save_path}")
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    if player:
        player.write_log("CYBER-CODE: Taking screenshot for analysis...")

    print("[PORZA-DEV] 📸 Capturing screen for debug...")
    ...
    if file_path:
        ...
        if err:
            print(f"[PORZA-DEV] ⚠️ Could not read file: {err}")
