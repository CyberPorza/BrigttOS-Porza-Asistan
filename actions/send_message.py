...
# CyberPorza Universal Messaging Module — WhatsApp, Instagram, Telegram vb.
...
def _open_app(app_name: str) -> bool:
    ...
    try:
        ...
        print(f"[PORZA-MSG] {app_name} açılamadı kankam: {e}")
        return False
...
def _send_whatsapp(receiver: str, message: str) -> str:
    ...
    try:
        if not _open_app("WhatsApp"):
            return "WhatsApp uygulamasını bulamadım veya açamadım kankam."
        ...
        return f"Mesaj {receiver} kişisine WhatsApp üzerinden gönderildi kankam."
...
def _send_instagram(receiver: str, message: str) -> str:
    ...
    try:
        ...
        return f"Mesaj {receiver} hesabına Instagram üzerinden fırlatıldı kankam."
...
def _send_telegram(receiver: str, message: str) -> str:
    ...
    try:
        if not _open_app("Telegram"):
            return "Telegram uygulamasını bulamadım kankam."
        ...
        return f"Mesaj {receiver} kişisine Telegram üzerinden iletildi kankam."
...
def _send_generic(platform: str, receiver: str, message: str) -> str:
    ...
    try:
        if not _open_app(platform):
            return f"{platform} uygulamasını açamadım kankam."
        ...
        return f"Mesaj {receiver} kişisine {platform} üzerinden gönderildi kankam."

    except Exception as e:
        return f"{platform} hatası: {e}"
...
def send_message(
    ...
    if not receiver:
        return "Mesajı kime göndereceğimi belirtmedin kankam."
    if not message_text:
        return "Ne mesajı göndereceğimi söylemedin kankam."

    print(f"[PORZA-MSG] 📨 {platform} → {receiver}: {message_text[:40]}")
    if player:
        player.write_log(f"CYBER-MSG: Sending to {receiver} via {platform}...")
    ...
    print(f"[PORZA-MSG] ✅ {result}")
    if player:
        player.write_log(f"CYBER-MSG: {result}")
