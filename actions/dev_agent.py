...
PROJECTS_DIR       = Path.home() / "Desktop" / "CyberPorzaProjects"
MAX_FIX_ATTEMPTS   = 4
...
Code for {file_path}:"""
    ...
    try:
        ...
        print(f"[PORZA-DEV] ✅ Written: {file_path}")
        return code
...
def _install_dependencies(dependencies: list[str], project_dir: Path) -> str:
    ...
    print(f"[PORZA-DEV] 📦 Installing: {dependencies}")
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    for cmd in vscode_paths:
        try:
            ...
            time.sleep(2)
            print(f"[PORZA-DEV] 💻 VSCode opened: {project_dir}")
            return True
        except Exception:
            continue
    print("[PORZA-DEV] ⚠️ VSCode not found.")
    return False


def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    """Run the project entry point, return output."""
    print(f"[PORZA-DEV] 🚀 Running: {run_command}")
...
Fixed code:"""
    ...
    try:
        ...
        print(f"[PORZA-DEV] 🔧 Fixed: {file_path}")
        return fixed
...
def _build_project(
    ...
    def log(msg: str):
        print(f"[PORZA-DEV] {msg}")
        if player:
            player.write_log(f"CYBER-DEV: {msg}")

    log("Planning project structure...")
    try:
        plan = _plan_project(description, language)
    except RateLimitError:
        msg = "API limitine takıldık kankam, biraz beklemen lazım."
        if speak: speak(msg)
        return msg
    ...
    proj_name = project_name or plan.get("project_name", "porza_project")
    ...
    if not file_codes:
        msg = "Bu proje için dosya oluşturamadım kankam."
        if speak: speak(msg)
        return msg
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if not _has_error(last_output):
            msg = (
                f"Proje '{proj_name}' çalışıyor kankam. "
                f"{attempt} denemede inşa edildi. "
                f"VSCode üzerinden {project_dir} yolunda açıldı."
            )
            if speak: speak(msg)
            return f"{msg}\n\nOutput:\n{last_output}"
    ...
    msg = (
        f"'{proj_name}' projesini {MAX_FIX_ATTEMPTS} denemeye rağmen çalışır hale getiremedim. "
        f"Proje {project_dir} yolunda kayıtlı — VSCode üzerinden manuel olarak kontrol edebilirsin."
    )
    if speak: speak(msg)
    return f"{msg}\n\nLast error:\n{last_output[:500]}"
...
def dev_agent(
    ...
    if not description:
        return "Geliştirmemi istediğin projeyi tarif eder misin kankam?"
