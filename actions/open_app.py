# actions/open_app.py
# CyberPorza — Cross-Platform App Launcher
...
def _launch_windows(app_name: str) -> bool:
    try:
        ...
        print(f"[PORZA-APP] ⚠️ Windows başlatma hatası: {e}")
        return False
...
def _launch_macos(app_name: str) -> bool:
    ...
    try:
        ...
        print(f"[PORZA-APP] ⚠️ macOS Spotlight hatası: {e}")
        return False
...
def open_app(
    ...
    if not app_name:
        return "Hangi uygulamayı açmamı istersin kankam?"

    system   = platform.system()
    launcher = _OS_LAUNCHERS.get(system)

    if launcher is None:
        return f"Desteklenmeyen işletim sistemi: {system}"

    normalized = _normalize(app_name)
    print(f"[PORZA-APP] 🚀 Başlatılıyor: {app_name} → {normalized} ({system})")

    if player:
        player.write_log(f"CYBER-APP: {app_name}")

    try:
        success = launcher(normalized)

        if success:
            return f"{app_name} başarıyla açıldı kankam."
        ...
        return (
            f"{app_name} uygulamasını açmayı denedim kankam, ama başlatıldığını doğrulayamadım. "
            f"Hala yükleniyor veya bilgisayarda kurulu olmayabilir."
        )

    except Exception as e:
        print(f"[PORZA-APP] ❌ {e}")
        return f"{app_name} açılamadı kankam: {e}"