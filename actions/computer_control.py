...
def _focus_window(title: str) -> str:
    ...
    if platform.system() == "Windows":
        try:
            # PowerShell injection önlemi için çift tırnakları kaçırıyoruz
...
def _analyze_screen_for_element(description: str) -> tuple[int, int] | None:
    ...
    try:
        ...
        print(f"[PORZA-CTRL] ⚠️ Screen analysis failed: {e}")
...
def computer_control(
    ...
    if player:
        player.write_log(f"CYBER-CTRL: {action}")

    print(f"[PORZA-CTRL] ▶️ Action: {action}  Params: {parameters}")
    ...
    try:
        if action == "type":
            ...
            print(f"[PORZA-CTRL] 🎲 Random {data_type}: {result}")
            return result
            ...
            if not value:
                ...
                print(f"[PORZA-CTRL] ⚠️ No user {field} in memory, using random: {value}")
            return value
