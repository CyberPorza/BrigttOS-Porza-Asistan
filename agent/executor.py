...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    ...
    if speak:
        speak("Bu görev için özel bir kod yazıyorum kankam.")
    ...
    try:
        ...
        code = re.sub(r"``" + r"`(?:python)?", "", code).strip().rstrip("`").strip()
        ...
        print(f"[PORZA-EXEC] 🐍 Running generated code: {tmp_path}")
    ...
    if tool == "file_controller" and params.get("action") in ("write", "create_file"):
        ...
        if not content or len(content) < 50:
            ...
            if all_results:
                ...
                print(f"[PORZA-EXEC] 💉 Injected + translated content")
...
def _translate_to_goal_language(content: str, goal: str) -> str:
    ...
    try:
        ...
        print(f"[PORZA-EXEC] 🌐 Translating to: {target_lang}")
        ...
        print(f"[PORZA-EXEC] ✅ Translation done ({target_lang})")
        return translated
    except Exception as e:
        print(f"[PORZA-EXEC] ⚠️ Translation failed: {e}")
        return content
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        ...
        print(f"[PORZA-EXEC] ⚠️ Unknown tool '{tool}' — falling back to generated_code")
...
class AgentExecutor:
    ...
    def execute(
        ...
        while True:
            ...
            if not steps:
                msg = "Bu görev için geçerli bir plan çıkaramadım kankam."
                if speak: speak(msg)
                return msg
            ...
            for step in steps:
                if cancel_flag and cancel_flag.is_set():
                    if speak: speak("Görev iptal edildi kankam.")
                ...
                print(f"\n[PORZA-EXEC] ▶️ Step {step_num}: [{tool}] {desc}")
                ...
                while attempt <= 3:
                    ...
                    try:
                        ...
                        print(f"[PORZA-EXEC] ✅ Step {step_num} done: {str(result)[:100]}")
                        step_ok = True
                        break

                    except Exception as e:
                        error_msg = str(e)
                        print(f"[PORZA-EXEC] ❌ Step {step_num} attempt {attempt} failed: {error_msg}")
                        ...
                        if decision == ErrorDecision.RETRY:
                            ...
                            print(f"[PORZA-EXEC] ⏭️ Skipping step {step_num}")
                            completed_steps.append(step)
                            step_ok = True
                            break

                        elif decision == ErrorDecision.ABORT:
                            msg = f"Görevi iptal ettim kankam. {recovery.get('reason', '')}"
                            if speak: speak(msg)
                            return msg
                            ...
                            if fix_suggestion and tool != "generated_code":
                                try:
                                    ...
                                    if speak: speak("Alternatif bir yol deniyorum kankam.")
                                    res = _call_tool(
                                        fixed_step["tool"],
                                    ...
                                    print(f"[PORZA-EXEC] ⚠️ Fix failed: {fix_err}")
            ...
            if replan_attempts >= self.MAX_REPLAN_ATTEMPTS:
                msg = f"Planı {replan_attempts} kez güncellememe rağmen görevi tamamlayamadım kankam."
                if speak: speak(msg)
                return msg

            if speak: speak("Yaklaşımımı değiştiriyorum kankam.")

            replan_attempts += 1
            plan = replan(goal, completed_steps, failed_step, failed_error)

    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None) -> str:
        fallback = f"İşlem tamam kankam. '{goal[:60]}' için {len(completed_steps)} adımı hallettim."
        try:
            import google.generativeai as genai
            genai.configure(api_key=_get_api_key())
            ...
            prompt    = (
                ...
                "Address the user as 'kankam' and reply in Turkish. Be direct and positive."
            )
            response = model.generate_content(prompt)
