...
PLANNER_PROMPT = """Sen Burak tarafından yaratılmış gelişmiş bir yapay zeka asistanı olan CyberPorza'nın planlama modülüsün.
    ...
    try:
        ...
        for step in plan["steps"]:
            if step.get("tool") in ("generated_code",):
                print(f"[PORZA-PLAN] ⚠️ generated_code detected in step {step.get('step')} — replacing with web_search")
                desc = step.get("description", goal)
                step["tool"] = "web_search"
                step["parameters"] = {"query": desc[:200]}

        print(f"[PORZA-PLAN] ✅ Plan: {len(plan['steps'])} steps")
        for s in plan["steps"]:
        ...
        print(f"[PORZA-PLAN] ⚠️ JSON parse failed: {e}")
        return _fallback_plan(goal)
    except Exception as e:
        print(f"[PORZA-PLAN] ⚠️ Planning failed: {e}")
        return _fallback_plan(goal)


def _fallback_plan(goal: str) -> dict:
    print("[PORZA-PLAN] 🔄 Fallback plan")
    return {
        "goal": goal,
        "steps": [
            {
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
    ...
    try:
        ...
        print(f"[PORZA-PLAN] 🔄 Revised plan: {len(plan['steps'])} steps")
        return plan
    except Exception as e:
        print(f"[PORZA-PLAN] ⚠️ Replan failed: {e}")
        return _fallback_plan(goal)