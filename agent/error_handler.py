...
ERROR_ANALYST_PROMPT = """You are the error recovery module of CyberPorza AI assistant.
...
Return ONLY valid JSON:
{
  ...
  "max_retries": 1,
  "user_message": "Short message in Turkish to tell the user, addressing them as 'kankam' (max 15 words)"
}
"""
...
def analyze_error(
    ...
    if attempt >= max_attempts:
        print(f"[PORZA-ERROR] ⚠️ Max attempts reached for step {step.get('step')} — forcing replan")
        return {
            "decision":      ErrorDecision.REPLAN,
            "reason":        f"Failed {attempt} times: {error[:100]}",
            "fix_suggestion": "Try a completely different approach or tool",
            "max_retries":   0,
            "user_message":  "Farklı bir yöntem deniyorum kankam."
        }
...
Attempt number: {attempt}"""
    ...
    try:
        ...
        if step.get("critical") and result["decision"] == ErrorDecision.SKIP:
            ...
            result["user_message"] = "Bu adım kritik kankam — alternatif bir yol arıyorum."

        print(f"[PORZA-ERROR] Decision: {result['decision'].value} — {result.get('reason', '')}")
        return result

    except Exception as e:
        print(f"[PORZA-ERROR] ⚠️ Analysis failed: {e} — defaulting to replan")
        return {
            "decision":       ErrorDecision.REPLAN,
            "reason":         str(e),
            "fix_suggestion": "Try alternative approach",
            "max_retries":    1,
            "user_message":   "Bir sorun çıktı kankam, yaklaşımımı değiştiriyorum."
        }
...
Return ONLY the Python code, no explanation."""
    ...
    try:
        ...
        print(f"[PORZA-ERROR] ⚠️ Fix generation failed: {e}")
        return {
            "step":        step.get("step"),
            "tool":        "generated_code",
